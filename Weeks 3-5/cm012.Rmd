...
- Notice also how levels on x-axis are placed in the "wrong" order. ###this is becuase deefault order everything by alphabetica order###
...
h_countries = c("Egypt", "Haiti", "Canada", "Afganistan", "Taiwan")
gap = gapminder %>%
...
gapminder %>%
  ...
  # reordering continent by max (median default) lifeExp
  coord_flip()+
  theme_bw() +
...
This might be useful if you are preparing a report for say, the state of affairs in Oceana.

```{r}
gapminder %>%
  ggplot() +
  geom_bar(aes(fct_relevel(continent, "Oceania", "Europe"))) +
  #orders by list of appearance here, rest is ordered alphabetically afterwards
  coord_flip()+
  theme_bw() +
  xlab("Continent") + ylab("Number of entries") 
```
...
gapminder %>%
  ...
  # can rename your factors using fct_recode() as defined above
  coord_flip()+
  theme_bw() +
  xlab("Continent") + ylab("Number of entries") 
